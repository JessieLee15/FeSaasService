...
## 开发环境
  * node 版本建议>=node v7.6.0，个人本地使用的v10.15.3
  * 执行npm install装依赖包

## 运行代码
  * npm run start
  * npm run debug(开发时推荐使用) 调试自行百度：chorme调试nodejs

## 环境及端口号
  * 本地开发使用3000端口：http://localhost:3000
...
│   └── retCode.js            //业务异常代码
├── data                   //静态数据（Json OBJ）
├── deploy                 //发布配置Jenkins、Dock
├── log4js.json            //koa-log4配置文件
├── logs                   //日志产出文件，项目启动自动生成
│   ├── app-2020-03-03.log     //普通日志
│   └── errors-2020-03-03.log  //异常日志
├── package.json          //项目npm信息文件
├── processes.json        //pm2发布配置文件
├── proxy                 //代理其他系统实现
├── pub                   //业务代码【主要开发代码都在这里】
│   ├── bll                   //业务逻辑代码，下面分模块
│   ├── model                 //预留目录：引入数据库后的结构之一
│   └── utils                 //业务开发中的公用工具库
├── routes                //路由：业务请求path的入口及出口
│   ├── baseData              //基础接口
│   ├── index.js              //路由首页【一层路由】
│   ├── mis                   //路由-模块（之一）
│   ├── service               //预留第三方服务路由模块
│   └── static.js             //静态数据直接读取路由实现：JSON格式
```

## 版本使用规范
* 发布版本在package.json version中体现，上线前请尽量去更新同步线上版本号及更新git tag
* 开发测试版本可在app.js第一层中间件中设置（全局参数：CUSTOM_VERSION），将会在首页路由请求到相关信息
  ```欢迎使用FE-Service!，版本：1.0.010，当前环境：development```
* 版本号使用规范：一共三段a.b.c
  * c为开发测试版本，三位数
  * b为日常上线版本，一位数开始累加
  * a为上线大功能版本，一位数开始累加
